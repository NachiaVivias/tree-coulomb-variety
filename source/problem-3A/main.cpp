...
vector<u32> subtree_query(const CentroidDecomposition::BFSUnit& tree){
    ...
    auto xB = get_dist_freq_BfO(tree); xB[0] = 0;
    auto xC = convolution<MOD, NTTzeta>(xA, xB);
    if(xC.size() > ans.size()) xC.resize(ans.size());
    for(int p : tree.I){
        int d = bfsbuf_dist[p];
        xC[d] = (xC[d] + (u64)bfsbuf_fIgp[p] * A[g] % MOD * B[p]) % MOD;
        if(p != g) xC[d] = (xC[d] + (u64)bfsbuf_fIpg[p] * A[p] % MOD * B[g]) % MOD;
    }
    return xC;
}
...
vector<u32> subtree_query_2(const CentroidDecomposition::BFSUnit& tree){
    ...
    auto xB = get_dist_freq_BfO(tree);
    auto xC = convolution<MOD, NTTzeta>(xA, xB);
    if(xC.size() > ans.size()) xC.resize(ans.size());
    get_bfsbuf_fIgp(tree);
    return xC;
}
...
int main() {
    ...
    for(int s=0; s<N; s++){
        ...
        //cout << "sigma_s = [ "; for(auto c : sigma_s) cout << c << " "; cout << "]" << endl;
        ...
        for(int nx : E[s]) if(centroid_decomposition.cdep[nx] > dep_s){
            ...
            //cout << "sigma_nx = [ "; for(auto c : sigma_nx) cout << c << " "; cout << "]" << endl;
